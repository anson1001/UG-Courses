...
\defin{Normalizable / Squared-Integrable}{
    A wave function $\Psi(x,t)$ is said to be \textbf{normalizable} (or \textbf{square-integrable} if it satisfies the condition
    \[
        \int_{-\infty}^{\infty} |\Psi(x,t)|^2 dx < \infty
    \]
}
Real particles can only be squared-integrable and thus normalizable.\\
\demo{
    We can normalize a squared-integrable wave function by dividing it by the square root of its norm, which is given by
    $$\psi_{norm}(x,t) = \frac{\psi(x,t)}{\sqrt{\int_{-\infty}^{\infty} |\psi(x,t)|^2 dx}}$$
}
...
Quantum theory is formulated on linear algebra, and we have the following correspondence:

\begin{table}[h]
    \centering
    \begin{tabular}{c|c}
        \textbf{Linear Algebra} & \textbf{Quantum Mechanics} \\
        \hline
        Vector & State Function \\
        Linear Operator & Observable \\
        Inner Product & Probability Amplitude \\
    \end{tabular}
\end{table}

All square-integrable, infinite-dimensional wave functions form a vector space. 
This vector space is called a \textbf{Hilbert space}, and it is a complete infinite-dimensional vector space equipped with a convergent inner product.

\defin{Hilbert Space}{
A complete infinite-dimensional vector space $L^2(a,b)$ equipped with a convergent inner product.
}
\defin{Inner Product}{
    ...
    \begin{itemize}
        ...
        \item Positive Definiteness: $\langle \psi | \psi \rangle \geq 0$ and $\langle \psi | \psi \rangle = 0$ if and only if $\psi = 0$
        \item Orthogonality: if $\langle \psi | \phi \rangle = 0$ then $\psi$ and $\phi$ are orthogonal
    \end{itemize}
}
\defin{Complete Set}{
    A set of functions $\{f_n(x)\}$ is said to be complete if,  $\forall \, f(x)$, we have
    $$ f(x) = \sum_{n=1}^{\infty} c_n f_n(x) $$
    where $c_n = \langle f_n | f \rangle$.
}
\ex{}{
    The set of eigenfunctions of the Hamiltonian operator (i.e. stationary states) forms a complete set.
}

\subsection{Observables}
\defin{Expectation Value of an Observable}{
    The expectation value of an observable $Q$ is given by
    $$ \langle Q \rangle = \langle \psi | \hat{Q} \psi \rangle $$
    \textbf{Properties}
    \begin{itemize}
        \item Observed values must be real, so $\langle Q \rangle = \langle Q \rangle^*$
        \item (following from the previous property) $\langle \psi | Q \psi \rangle = \langle Q \psi | \psi \rangle^*$
    \end{itemize}
}
\subsubsection{Hermitian Operators}
\defin{Hermitian Operator}{
    An operator $\hat{Q}$ is said to be Hermitian if
    $$ \langle \psi | \hat{Q} \phi \rangle = \langle \hat{Q} \psi | \phi \rangle $$
    for all $\psi$ and $\phi$ in the Hilbert space.
}
\ex{}{Observables are always hermitian.}
\defin{Hermitian Conjugate}{
    The Hermitian conjugate of an operator $\hat{Q}$ is denoted by $\hat{Q}^\dagger$ such that
    $$ \langle \psi | \hat{Q} \phi \rangle = \langle \hat{Q}^\dagger \psi | \phi \rangle $$
    \textbf{Properties}
    \begin{itemize}
        \item For hermitian operators, $\hat{Q} = \hat{Q}^\dagger$
        \item $(\hat{Q}^\dagger)^\dagger = \hat{Q}$
    \end{itemize}
}
\subsubsection{Determinate States}
Determinate states are states that return the same value when measured, i.e. $\sigma = 0$, wich is equivalent to
$$ \hat{Q} \psi = q \psi $$
for some $q$.
\defin{Determinate State}{
    A state $\psi$ is said to be a determinate state of an observable $Q$ if $\psi$ is an eigenstate of the operator $\hat{Q}$, i.e. $\hat{Q} \psi = q \psi$ where $q$ is the eigenvalue.
}
